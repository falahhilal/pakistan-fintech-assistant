...
if not os.path.exists("vectorstore"):
    ...
    with st.spinner("Building knowledge base for the first time... please wait (~2 mins)"):
        from langchain_community.document_loaders import PyPDFDirectoryLoader
        from langchain_text_splitters import RecursiveCharacterTextSplitter
        from langchain_community.vectorstores import Chroma as ChromaBuilder
...
Return ONLY the rewritten question, nothing else."""),
    ...
    if st.session_state.suggested:
        ...
        st.session_state.suggested = None
    elif prompt := st.chat_input("Ask about Pakistan's fintech, payments, budget, or financial inclusion...", key="main_input"):
        user_input = prompt
